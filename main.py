...
#S3 credentials
access_key = 'AKIA34D3YCMEI723IPH7'
access_secret = '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'
bucket_name = 'user1folder'
...
def transform_and_save(image, base_path, image_id, current_user=current_user):
    ...
    client_s3.upload_file(main_image_path,bucket_name,f"uploads/{current_user.name}/{image_id}/main/{unique_filename}")
    ...
    blur_and_save(main_image_path, blur_folder, unique_filename, image_id)

    # Create Shade
    shade_folder = f"{base_path}/{current_user.name}/{image_id}/shade/"
    os.makedirs(shade_folder, exist_ok=True)
    shade_and_save(main_image_path, shade_folder, unique_filename, image_id)

    # Create Spread
    spread_folder = f"{base_path}/{current_user.name}/{image_id}/spread/"
    os.makedirs(spread_folder, exist_ok=True)
    spread_and_save(main_image_path, spread_folder, unique_filename, image_id)
...
def thumbnail_and_save(source_image_path, destination_folder, filename ):
    ...
    client_s3.upload_file(f"{destination_folder}{filename}",bucket_name,f"uploads/{current_user.name}/thumbnails/{filename}")


def blur_and_save(source_image_path, destination_folder, filename, image_id):
    with Wand_Image(filename=source_image_path) as img:
        ...
        client_s3.upload_file(f"{destination_folder}{filename}",bucket_name,f"uploads/{current_user.name}/{image_id}/blur/{filename}")


def shade_and_save(source_image_path, destination_folder, filename, image_id):
    with Wand_Image(filename=source_image_path) as img:
        img.shade(gray=True, azimuth=286.0, elevation=45.0)
        img.save(filename=f"{destination_folder}{filename}")
        client_s3.upload_file(f"{destination_folder}{filename}",bucket_name,f"uploads/{current_user.name}/{image_id}/shade/{filename}")


def spread_and_save(source_image_path, destination_folder, filename, image_id):
    with Wand_Image(filename=source_image_path) as img:
        img.spread(radius=8.0)
        img.save(filename=f"{destination_folder}{filename}")
        client_s3.upload_file(f"{destination_folder}{filename}",bucket_name,f"uploads/{current_user.name}/{image_id}/spread/{filename}")
